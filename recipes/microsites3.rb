    ...
    if any_app['id'] == role
      ...
      if app['type'][app['id']].include?( "upstream_microsites3" )
        ...
        app['services'].each do |service|
          template "/lib/systemd/system/#{service['name']}.service" do
            source "lib/systemd/system/microsites3.service.erb"
            owner "root"
            group "root"
            mode "0664"
            variables(
              ...
              :cwd         => "#{app['deploy_to']}/current",
              :exec_start  => "/home/#{user}/.rbenv/versions/#{ruby_version}/bin/bundle exec rake #{service['rake_task']}",
              :environment => app['rack_environment']
            )
          end
          service "#{service['name']}.service" do
            supports :status => true, :restart => true
            action [ :enable, :start, :restart ]
          end
        end
